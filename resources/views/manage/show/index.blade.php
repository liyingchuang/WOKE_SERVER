...
<div class="panel  panel-info">
    ...
    <div class="panel-body">
       <form action="{{URL::to('manage/show')}}" method="get">
   ...
   <form action="{{URL::to('manage/show')}}" method="post">
    ...
    <tr  @if(count($u->report)>0)  class="danger" @endif >
          <td scope="row"><label> <input type="checkbox" class="t" name="ids[]" value="{{$u->id}}"><span class="text"></span>
	</label></td>
        <td>{{$k+1}}</td>
         <td>
             <a href="{{$u->file_name}}" target="_break" ><img src="{{$u->file_name}}?imageView2/2/w/50" alt="..." class="img-thumbnail"></a>
         </td>
         <td>@if(!empty($u->user))<a href="{{URL::to('manage/show/')}}?id={{$u->user->user_id}}"   > <img id="avatar" src="{{$u->user->headimg}}?imageView2/2/w/50" alt="" class="img-circle" width="40"></a>
         {{$u->user->user_name}}  @if($u->user->is_v)<span style="font-weight:bold;font-style:italic;color:red"> V</span> @endif @endif
         </td>
        <td width="10%">
            @foreach ($u->tags as $ktag=>$utag)
                <a class='btn btn-default'  href="javascript:;">{{$utag->tag_name}} | {{$utag->size}}</a>
            @endforeach
        </td>
         <td>
           <label>
        ...
        <td>
            <label>
                <input class="checkbox-slider slider-icon colored-darkorange offon" type="checkbox"  title="{{$u->id}}" @if($u->is_recommend == 2) checked @endif>
                <span class="text"></span>
            </label>
        </td>
        <td>{{$u->created_at}}</td>
         <td class="center ">
             <button class="btn btn-danger  btn-xs "  type="button"  onclick="if(confirm('确定删除吗?')) {
                 ...
                 var obj = $(this); 
                $.get('{{ url('manage/show') }}/{{$u->id}}/edit', function(response){
    ...
    <div class="panel-footer"> 
        <div class="row" >
            <div class="col-sm-5 col-xs-5 col-md-5 " >
                <button class="btn btn-danger  btn-xs " type="submit"><i class="glyphicon glyphicon-trash"></i> 删除</button>
            </div>
