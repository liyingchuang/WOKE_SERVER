		...
		<div class="panel-body">
			<div class="tabbable">
				<div class="tab-content">
					<form method="post"  action="{{URL::to('manage/prize/addactivity')}}" class="form-horizontal" id="html5Form" data-bv-message="数据不能为空" data-bv-feedbackicons-valid="glyphicon glyphicon-ok" data-bv-feedbackicons-invalid="glyphicon glyphicon-remove" data-bv-feedbackicons-validating="glyphicon glyphicon-refresh" >
						<div class="form-group">
							<label for="uuid" class="col-sm-2 col-xs-2 col-md-2 control-label no-padding-right">活动名称:</label>
							<div class="col-sm-3 col-xs-3 col-md-3">
								<input type="text" class="form-control required safe-input" name="name" id="name" title="输入活动名称" placeholder="输入活动名称" required="" data-bv-notempty-message="活动名称空">
							</div>
							<label for="uuid" class="col-sm-2 col-xs-2 col-md-2 control-label no-padding-right">活动所需积分:</label>
							<div class="col-sm-3 col-xs-3 col-md-3">
								<input type="text" class="form-control required safe-input" name="size" id="size" title="输入活动所需积分" placeholder="输入活动所需积分" required="" data-bv-notempty-message="输入活动所需积分">
							</div>
						</div>
						<div class="form-group">
							<label for="uuid" class="col-sm-2 col-xs-2 col-md-2 control-label no-padding-right">开始时间:</label>
							<div class="col-sm-3 col-xs-3 col-md-3">
								<input type="text" class="form-control required date-picker minView: 2" name="start_time" id="start_time"  title="开始时间" placeholder="活动开始时间"
							...
							<div class="col-sm-3 col-xs-3 col-md-3">
								<input type="text" class="form-control required date-picker" name="end_time" id="end_time"  title="结束时间" placeholder="活动结束时间"
									   data-fv-date="true" required=""
							...
							<div class="col-sm-8 col-xs-8 col-md-8">
								<table class="table  table-bordered table-striped">
									...
									@foreach ($prize_goods as $key => $val)
										<tr>
											<td scope="row"><label> <input type="checkbox" name="chk_role[]" value="{{ $val->id }}" class="multi_checked"><span class="text"></span></label></td>
											<td>{{$key+1}}</td>
											<td>{{$val->name}}</td>
											<td>
												@if(!empty($val->image))
											...
											<td><input type="text" name="probability[{{ $val->id }}]" class="multi_checked" style="width:35px;" placeholder="概率">%</td>
										</tr>
									@endforeach
									</tbody>
								</table>
