    ...
    <form method="post"  class="form-horizontal form-bordered" >
     <div class="form-group">
            ...
            <div class="col-sm-8 col-xs-8 col-md-8">
				...
				<input type="hidden" name="id" id="id" value="{{ $info_prize_orders->id }}">
                  <table class="table  table-bordered table-striped float">
                    <tbody>
						...
						<tr>
							 ...
							 <td>{{ $info_prize_orders->order_sn }}</td>
							 <td align="right">订单状态：</td>
							 <td>
								@if( $info_prize_orders->shipping_status == 0) 未发货 @elseif( $info_prize_orders->shipping_status == 1) 已发货 @elseif( $info_prize_orders->shipping_status == 2) 收货确认 @else 备货中  @endif
							 </td>
                         </tr>
						 <tr>
							 <td align="right">购货人：</td>
							 <td>{{ $info_prize_orders->username }}</td>
							 <td align="right">下单时间：</td>
							 <td>{{ $info_prize_orders->created_at }}</td>
                         </tr>
						 <tr>
							 <td align="right">发货单号：</td>
							 <td>{{ $info_prize_orders->shipping_number }}</td>
							 <td align="right">发货时间：</td>
							 <td>@if( $info_prize_orders->shipping_status == 0) 未发货 @else {{ $info_prize_orders->shipping_time }}  @endif</td>
                        </tr>
						<tr>
							<td align="right">配送方式：</td>
							<td>{{ $info_prize_orders->shipping_name }}</td>
							 <td align="right">收货人信息：</td>
							 <td >
								{{ $info_prize_orders->address }}
							 </td> 
                        </tr>
                    </tbody>
                  </table>
            ...
            <div class="col-sm-8 col-xs-8 col-md-8">
                ...
                <div class="table-responsive" style="margin-top:0px;">
                  <table class="table  table-bordered table-striped">
                    ...
                    <tbody>
						@foreach ($info_prize_orders_goods as $key=>$val)
                        <tr>
							 <td>{{ $val->name }}</td>
							 <td>
								 @if(!empty($val->image))
                  ...
                  <table class="table  table-bordered table-striped float">
                    <tbody>
						...
						<tr>
							...
							<td>
								@if( $info_prize_orders->shipping_status == 0)
								<div>
									<div class="col-sm-3">
									...
									<div class="col-sm-2">
										<select name="shipping_express" id="shipping_express" class="form-control">
											<option value=""  @if( $info_prize_orders->username != '顺丰速运' ) selected @endif>快递选择......</option>
											<option value="圆通速递">圆通速递</option>
											<option value="申通快递">申通快递</option>
											<option value="中通速递">中通速递</option>
											<option value="韵达快递">韵达快递</option>
											<option value="天天快递">天天快递</option>
											<option value="顺丰速运" @if( $info_prize_orders->username == '顺丰速运' ) selected @endif>顺丰速运</option>
