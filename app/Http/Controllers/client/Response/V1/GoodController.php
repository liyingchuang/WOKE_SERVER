...
class GoodController extends BaseResponse implements InterfaceResponse
{
    ...
    public function android(Request $request)
    {
        ...
        $info = Goods::with('attr', 'item', 'gallery','store')->select('goods_id','goods_sn','supplier_id',"goods_name",'is_real','integral',"goods_name_style","click_count","goods_number","goods_weight","goods_texture","goods_norms","goods_author","goods_area", "goods_style","market_price", "shop_price","goods_desc","goods_thumb","original_img")->where('goods_id', $goods_id)->where('is_delete', 0)
...
*/
    ...
    public function index(Request $request)
    {
        $user_id = $request->get('user_id');
        $goods_id = $request->get('goods_id');
        $info = Goods::with('attr', 'item', 'gallery','store')->select('goods_id','supplier_id','goods_sn',"goods_name",'is_real','integral',"goods_name_style","click_count","goods_number","goods_weight","goods_texture","goods_norms","goods_author","goods_area", "goods_style","market_price", "shop_price","goods_desc","goods_thumb","original_img")->where('goods_id', $goods_id)->where('is_delete', 0)
            ->where('is_on_sale', 1)->where('goods_number','>',0)->first();
        if(empty($info)){
            return $this->error(null,'商品不存在!或已经下架');
        }
