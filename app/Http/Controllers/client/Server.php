...
class Server
{
    ...
    public function run()
    {
        ...
        // C. 校验签名
        $method=self::getMethodName($this->method);
        if($method!='select'){
        ...
        $methodName=self::getMethodName($this->method);
        // D.3 判断方法是否存在
        ...
        // E. api接口分发
        $class = new $classPath();
        if(!$class->checkToken($this->request)){
           return $this->response(['status' => false, 'code' => '1030']);
        }
        return $this->response((array) $class->$methodName($this->request));
    }
    ...
    protected function checkSign($params)
    {
        ...
        unset($params['sign']);
        //echo $this->generateSign($params) ;exit;
        $debut = env('APP_DEBUG');
        if (!$debut) {
