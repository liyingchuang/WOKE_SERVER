...
class Address extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:address';
    ...
    {
        $add = DB::table('woke_user_address')->select('address_id', 'province', 'address')->get();
        foreach($add as $item) {
            if($item->province == 1){
            if($item->address){
                if(strpos($item->address,'-')){
                    ...
                    if(count($exp) > 1){
                        ...
                        $this->info($item->address_id.'Error');
                    }
                }
                if(count($exp) > 1){
                    ...
                    DB::table('woke_user_address')->where("address_id", $item->address_id)->update(['province'=>$exp[0], 'city'=>$exp[1], 'district'=>$exp2[0], 'address'=>$exp2[1]]);
                    }else{
                        $this->info($item->address_id.'Error');
                    }
                }else{
                    $this->info($item->address_id.'Null');
                }
            }
        }
    }
